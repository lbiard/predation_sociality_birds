...
Note that `output_ericson.nex` and `output_hackett.nex` are distributions of phylogenetic trees and are too large to be stored on GitHub, but they will present in the final repository (e.g. on Zenodo or Dryad). These files are only needed to reproduce some results present in the Appendix.
